...
cd ${PrjDir}/build
cmake -G"MinGW Makefiles" -DPRJOUTPUT=%PrjOutput% -DANDROID=true -DIOS=false -DCMAKE_TOOLCHAIN_FILE=%ToolsDir%android-cmake/android.toolchain.cmake -DTOOLS_DIR=%ToolsDir%CMakeUtility -DCMAKE_MAKE_PROGRAM="%ANDROID_NDK%\prebuilt\windows\bin\make.exe" -DANDROID_ABI=%ANDROID_ABI% -DANDROID_NATIVE_API_LEVEL=%ANDROID_NATIVE_API_LEVEL% ..
if [ "$?" != "0" ]; then
    exit $?
fi
